...
#
# * 18/11/2007 v1.54 Newer
#   PhoenixX mods added - BUY NOW, + BUY OUT, ability to disable delete post
#   Version available from cybernexus.biz on request
#
# * 13/04/2008 v1.54 Fixes
#   Bugfix allow no reserve auction, Bug fix + option seller can't bid on own auction unless allowed
#   Registration will now notify on screen what your login is for hosts that wont send reg email
#
# * MM/YYYY
...
$config{'basepath'} = '/home//www/pc.cybernexus.biz/auctiondata/';
#'f:/home/www/pc.cybernexus.biz/auctiondata/';
...
%category = (
	...
	gameitems => 'Game Items Real Money',
	gameitemsG => 'Game Items Game Money',
	minions => 'Staff and Minions Game Money',
	gizmo => 'Inventions and Gizmos'
);

# This is the password for deleting auction
# items.
# Use Blank to disable if you plan to delete posts from a console

$config{'adminpass'} = '';

# Ability for sellers to bid on own auction remove comment to allow
# $config{'vendorbid'} = 'Yes';
...
#$config{'mailhost'} = 'mail.supremecluster.com';

# This line should be your e-mail address

$config{'admin_address'} = 'noreply@cybernexus.biz';
...
$config{'scripturl'} = 'pc.cybernexus.biz';
...
$config{'sitename'} = 'Open Auction Site';
...
$config{'footer'} =<<"EOF";
<P>
<CENTER><FONT SIZE=-1><I>Powered By <A HREF=http://www.everysoft.com/auction/>EveryAuction 1.54(+buynow+buyout+admin mod)</A></I> Contact sales at cybernexus dot biz if you want it.</FONT></CENTER>
</BODY>
</HTML>
EOF
...
sub displist {
	...
	print "<TR><TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>Item</B></TD>
               <TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>Closes</B></TD>
               <TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>Num Bids</B></TD>
               <TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>High Bid</B></TD>
               <TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>Type</B></TD>
               </TR>\n";
	...
	foreach $file (@allfiles) {
		...
		$file =~ s/\.dat$//;
		my ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids) = &read_item_file($form{'category'},$file);
		if ($title ne '') {
			...
			print " <!FONT COLOR=#3333FF SIZE=-1><img height=80 src=$image><!/FONT>" if ($image);
			print "</TD><TD BGCOLOR=$config{'colortablebody'}>$closetime[3]/$closetime[4]/" . ($closetime[5] += 1900) . " $closetime[2]:$closetime[1]:$closetime[0]</TD>
                        <TD BGCOLOR=$config{'colortablebody'}>$#bids</TD>
                        <TD BGCOLOR=$config{'colortablebody'}>\$$bid</TD>
                        <TD BGCOLOR=$config{'colortablebody'}><FONT COLOR=#3333FF SIZE=-1>";
                        if ($buynow > 0) { if ($config{'regdir'}) { print "BUYNOW!"; } }
                        else { if ($buyout eq "YES") { print "BUYOUT at \$$reserve"; if ($bid >= $reserve) { print "<b> SOLD!</b>"; &closeit($form{'category'},$file); } }
                               else { print "Auction";  print "<br><FONT SIZE=-1>(reserve not met)</FONT>" if ($bid < $reserve); }
                               }
                        print "</FONT></TD></TR>\n";
...
sub dispitem {
        my ($sellercheck);
        my ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids) = &read_item_file($form{'category'},$form{'item'});
	...
	print "<TD><TABLE BORDER=1><TR><TD BGCOLOR=$config{'colortablehead'}><B>$title</B></TD></TR><TR><TD BGCOLOR=$config{'colortablebody'}><B>Category:</B> <A HREF=$ENV{'SCRIPT_NAME'}\?category=$form{'category'}>$category{$form{'category'}}</A></TD></TR><TR><TD BGCOLOR=$config{'colortablebody'}>";
	my ($alias, $email, $bid, $time, $add1, $add2, $add3) = &read_bid($bids[0]); # read first bid
	print "<B>Offered By:</B> <A HREF=mailto:$email>$alias</A></TR></TD>
              <TR><TD BGCOLOR=$config{'colortablebody'}><B>Current Time:</B> $nowtime</TD></TR>
              <TR><TD BGCOLOR=$config{'colortablebody'}><B>Closes:</B> $closetime<BR><FONT SIZE=-2>Or $config{'aftermin'} minutes after last bid...</FONT></TD></TR>
              <TR><TD BGCOLOR=$config{'colortablebody'}>";
	 ($alias, $email, $bid, $time, $add1, $add2, $add3) = &read_bid($bids[$#bids]); # read last bid
	#it is a buy now buy out or reserve?
	if(($bid == $reserve) and ($buynow == $reserve)) {
             print "<B>BUY ME NOW!</B></TD></TR><TR><TD BGCOLOR=$config{'colortablebody'}><B>\$$bid</B>";
        } else {
	     print "<B>Number of Bids:</B> $#bids</TD></TR><TR><TD BGCOLOR=$config{'colortablebody'}>";
             print "<B>Last Bid:</B> \$$bid ";
	     print "<FONT SIZE=-1>(reserve " if (($reserve>0) and ($buyout ne 'YES'));
             print "<FONT SIZE=-1>(buy out $reserve " if (($reserve>0) and ($buyout eq 'YES'));
             print "price not yet met)</FONT>" if ($bid < $reserve);
	     print "price met)</FONT>" if (($bid >= $reserve) and ($reserve > 0));
	}
	...
	($alias, $email, $bid, $time, $add1, $add2, $add3) = &read_bid($bids[$#bids]); # read the last bid
        my ($sold)='';
        #did we buy it out, or buy now?
        if (($buyout eq 'YES') and ($bid >= $reserve) and ($reserve > 0)) { $sold='yes'; }
        
        if ((time > int($form{'item'})) && (time > (60 * $config{'aftermin'} + $time)) || ($sold eq 'yes'))  {
		print "<FONT SIZE=-1 COLOR=#FF0000><B>BIDDING IS NOW CLOSED</B></FONT><BR>";
		&closeit($form{'category'},$form{'item'});
	}
	else {
        $sellercheck=$alias;
        $sellercheck =~ tr/[a-z]/[A-Z]/;
		print <<"EOF";
<FORM ACTION=$ENV{'SCRIPT_NAME'} METHOD=POST>
...
<INPUT TYPE=HIDDEN NAME=SELLER VALUE="$sellercheck">
...
EOF
                          if(($bid == $reserve) and ($buynow == $reserve)) {
                                   print '<INPUT NAME=BID TYPE=HIDDEN SIZE=7 VALUE="' . $lowest_new_bid .'">';
                          } else { print '<BR><B>Your Bid:</B> $<INPUT NAME=BID TYPE=TEXT SIZE=7 VALUE="' . $lowest_new_bid .'">'; }
...
EOF
                   	...
                   	print '<P><B>Contact Information:</B> (will be given out only to the seller)<BR>
		...
		if(($bid == $reserve) and ($buynow == $reserve)) {
		         print '<br><INPUT TYPE=SUBMIT VALUE="BUY NOW!">';
		} else { print '<INPUT TYPE=SUBMIT VALUE="Place Bid">'; }
	}
}
...
# PhoenixX mod 17/11/07
# added support for buynow and buyout

sub new {
	my ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids);
	...
	if ($form{'REPOST'}) {
		...
		if (-T "$config{'basepath'}$config{'closedir'}/$form{'REPOST'}.dat") {
			...
			($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids) = <THEFILE>;
			close THEFILE;
			chomp($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids);
		}
	}
	print <<"EOF";
<FORM ACTION=$ENV{'SCRIPT_NAME'} METHOD=POST>
...
<TR><TD VALIGN=TOP><B>Days Until Close:<BR></B>1-28</TD><TD><INPUT NAME=DAYS TYPE=TEXT SIZE=2 MAXLENGTH=2></TD></TR>
...
<TR><TD VALIGN=TOP><B>_OR_ Your Buy Now Price:<BR></B>Leave 0 if not needed. Note: Buy now means no bids! It sells immediately!! <br><b>WARNING: Setting this will overrule any other settings!</b></TD><TD>\$<INPUT NAME=BUYNOW TYPE=TEXT SIZE=7 VALUE=0>
<TR><TD VALIGN=TOP><B>Your Reserve Price:<BR></B>You are not obligated to sell below this price.  Leave blank if none.<br>Buyout means it sells once a bid reaches this point! When using buyout you should sell at the final bid price.</TD><TD>\$<INPUT NAME=RESERVE TYPE=TEXT SIZE=7 VALUE=0>
BuyOut At Reserve?<SELECT NAME=BUYOUT><OPTION SELECTED>NO</OPTION><OPTION>YES</OPTION>
...
sub procnew {
	...
	&oops('You must enter the number of days your auction should run, from 1 to 28.') unless (($form{'DAYS'} > 0) and ($form{'DAYS'} < 29));
	...
	&oops('You must enter a valid starting bid or buy now price.') unless (($form{'BID'} =~ /^(\d+\.?\d*|\.\d+)$/) || ($form{'BUYOUT'} =~ /^(\d+\.?\d*|\.\d+)$/));
	&oops('You must enter a starting bid lower than the reserve price!') unless (($form{'RESERVE'} >= $form{'BID'}) || (!$form{'RESERVE'})) ;
	...
	$form{'BUYNOW'} = &parsebid($form{'BUYNOW'});
	# if we enter a buy now price it overrules the bid start price so they need to be the same.
        #buyout uses same close test so reserve needs to equal buyout price
        if (&parsebid($form{'BUYNOW'}) >0) { $form{'BID'} = $form{'BUYNOW'}; $form{'RESERVE'} = $form{'BID'}; $form{'BUYOUT'}='NO'; }
        
 	&oops('You must enter your buyout option! If using auto create auction tool, it must support BUYOUT YES or NO.') unless ($form{'BUYOUT'});
	...
	if ($form{'FROMPREVIEW'}) {
		...
		print "<TD><TABLE BORDER=1><TR><TD BGCOLOR=$config{'colortablehead'}><B>$form{'TITLE'}</B></TD></TR>
                       <TR><TD BGCOLOR=$config{'colortablebody'}><B>Category:</B><A HREF=$ENV{'SCRIPT_NAME'}\?category=$form{'CATEGORY'}>$category{$form{'CATEGORY'}}</A></TD></TR>
                       <TR><TD BGCOLOR=$config{'colortablebody'}><B>Offered By:</B> <A HREF=mailto:$form{'EMAIL'}>$form{'ALIAS'}</A></TR></TD>
                       <TR><TD BGCOLOR=$config{'colortablebody'}><B>Current Time:</B> $nowtime</TD></TR>
                       <TR><TD BGCOLOR=$config{'colortablebody'}><B>Closes:</B> $closetime<BR><FONT SIZE=-2>Or $config{'aftermin'} minutes after last bid...</FONT></TD></TR>
                       <TR><TD BGCOLOR=$config{'colortablebody'}><B>Number of Bids:</B> 0</TD></TR>
                       <TR><TD BGCOLOR=$config{'colortablebody'}><B>Bid/Price:</B> \$$form{'BID'}</TD></TR></TABLE></TD></TR></TABLE>\n";
		print "<HR><FONT SIZE=+1><B>Description</B></FONT><HR>$html_description</FONT></FONT></B></I></U></H1></H2></H3></H4></H5>";
...
sub procbid {
	my ($password, @userbids);
	my $sellercheck = $form{'SELLER'};
	my $aliascheck = $form{'ALIAS'};
	if ($config{'regdir'} ne "") {
		&oops('Your alias could not be found!') unless ($password, $form{'EMAIL'}, $form{'ADDRESS1'}, $form{'ADDRESS2'}, $form{'ADDRESS3'}, @userbids) = &read_reg_file($form{'ALIAS'});
		$form{'ALIAS'} = ucfirst(lc($form{'ALIAS'}));
		&oops('Your password is incorrect.') unless ((lc $password) eq (lc $form{'PASSWORD'}));
        ...
        $sellercheck  =~ tr/[a-z]/[A-Z]/;
        $aliascheck =~ tr/[a-z]/[A-Z]/;
	if (!$config{'vendorbid'}) { &oops('You are not allowed to bid on your own auction!') unless ($aliascheck ne $sellercheck); }
	&oops('You must enter an alias to track your item.') unless ($form{'ALIAS'});
	&oops('You must enter a valid e-mail address.') unless (&check_email($form{'EMAIL'}));
	...
	if ((time <= $form{'ITEM'}) or (time <= (60 * $config{'aftermin'} + $time))) {
		...
		print "<B>$form{'ALIAS'}, your bid has been placed on item number $form{'ITEM'} for \$$form{'BID'} on ".scalar(localtime(time)).".</B><BR>You may want to print this notice as confirmation of your bid.<P>Go <A HREF=\"$ENV{'SCRIPT_NAME'}\?category=$form{'CATEGORY'}\&item=$form{'ITEM'}\">back to the item</A>\n";
                #Was it a Buy Now Bid? Close Item if so
                if (($reserve) and ($bid >= $reserve) and ($buynow == $reserve)) { &closeit($form{'CATEGORY'},$form{'ITEM'}); print "<BR><H2>You Purchased this item!!!</H2><br>"; }
...
###########
# Sub parseline - assembles each item line for search results
# Added by PhoenixX 18/11/2007

sub parseline {
    my ($bid, $key, $file, $title, $reserve, $inc, $desc, $image, $buynow, $buyout, $bids, $month, $day, $year, $hour, $min, $sec) = @_;
    my $line = "<TR><TD BGCOLOR=$config{'colortablebody'}><A HREF=$ENV{'SCRIPT_NAME'}\?category=$key\&item=$file>$title</A>";
    if ($image) { $line=$line . "<img height=80 src=$image>"; }
    $line=$line . "</TD><TD BGCOLOR=$config{'colortablebody'}>$day/$month/". ($year += 1900) .
                  " $hour:$min:$sec</TD><TD BGCOLOR=$config{'colortablebody'}>$bids</TD>
                   <TD BGCOLOR=$config{'colortablebody'}>\$$bid</TD>
                   <TD BGCOLOR=$config{'colortablebody'}><FONT COLOR=#3333FF SIZE=-1>";

    if ($buynow > 0) { if ($config{'regdir'}) { $line=$line . "BUYNOW!"; } }
           else { if ($buyout eq "YES") { $line=$line . "BUYOUT at \$$reserve"; if ($bid >= $reserve) { $line=$line . "<b> SOLD!</b>"; &closeit($key,$file); } }
                else { $line=$line . "Auction";  $line=$line . "<br><FONT SIZE=-1>(reserve not met)</FONT>" if ($bid < $reserve); }
                }
           $line=$line . "</FONT></TD></TR>\n";
    return "$line";
}
#if ($bid >= $reserve) { print "<b> SOLD!</b>"; &closeit($form{'category'},$file); }
#-#############################################
# Sub: Process Search
...
sub procsearch {
	...
	print "<TR><TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>Item</B></TD>
        ...
        <TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>Num Bids</B></TD>
        <TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>High Bid</B></TD>
        <TD ALIGN=CENTER BGCOLOR=$config{'colortablehead'}><B>Type</B></TD></TR>\n";
	my $key;
	foreach $key (sort keys %category) {
		...
		foreach $file (@allfiles) {
			...
			$file =~ s/\.dat$//;
			my ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids) = &read_item_file($key,$file);
			if ($title ne '') {
				my ($alias, $email, $bid, $time, $add1, $add2, $add3) = &read_bid($bids[$#bids]);
				my @closetime = localtime($file);
				$closetime[4]++;
				if($form{'searchtype'} eq 'keyword' and ($title =~ /$form{'searchstring'}/i) || ($desc =~ /$form{'searchstring'}/i)) {
		                        print &parseline($bid, $key, $file, $title, $reserve, $inc, $desc, $image, $buynow,
                                         $buyout, $#bids, $closetime[4],$closetime[3],$closetime[5],$closetime[2],$closetime[1],$closetime[0]);
				}
...
sub procreg {
	if ($config{'regdir'}) {
		...
		if (!(-f "$config{'basepath'}$config{'regdir'}/$form{'ALIAS'}.dat")) {
			...
			close NEWREG;
			print "$form{'ALIAS'}, you may receive an e-mail to $form{'EMAIL'} in a few minutes.  It will contain your password needed to post or bid.  You may change your password once you receive it.  If you do not get an e-mail, a copy is below.\n";
			print '<pre>' . "PLEASE DO NOT REPLY TO THIS E-MAIL.\r\n\r\nThank you for registering to use the online auctions at $config{'sitename'}!\r\n\r\nYour new password is: $newpass\r\nYour alias (as you entered it) is: $form{'ALIAS'}\r\n\r\nThank you for visiting!" . '</pre>';
			&sendemail($form{'EMAIL'}, $config{'admin_address'}, 'Auction Password', "PLEASE DO NOT REPLY TO THIS E-MAIL.\r\n\r\nThank you for registering to use the online auctions at $config{'sitename'}!\r\n\r\nYour new password is: $newpass\r\nYour username alias (as you entered it) is: $form{'ALIAS'}\r\n\r\nThank you for visiting!");
		}
		else {
...
sub viewclosed2 {
	...
	foreach $bid (@past_bids) {
		if (-T "$config{'basepath'}$config{'closedir'}/$bid.dat") {
			...
			my ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids) = <THEFILE>;
			close THEFILE;
			chomp($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids);
...
sub procadmin {
	...
	if (lc($form{'PASSWORD'}) eq lc($config{'adminpass'})) {
     ...
     } else { print "Sorry... the delete function has been disabled by the auction administrator!\n"; }
}

#-#############################################
# Sub: Close Auction
# This sets an item's status to closed.\
# 17/11/07 PhoenixX Mods
# Added $buynow flag check and $buyout bid check
# buynow is no bid win immediately buyout is bid up to or buy immediately
...
sub closeit {
	...
	if ($cat ne $config{'closedir'}) {
		my ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids) = &read_item_file($cat,$item);
		my @lastbid = &read_bid($bids[$#bids]);
		my @firstbid =  &read_bid($bids[0]);

		if ($#bids) {
			if (($lastbid[2] >= $reserve) || ($buynow) || ($lastbid[2] == $buyout)) {
...
# Reads an item file
# PhoenixX Mod 17/11/07
# added $buynow, $buyout, to file layout
...
sub read_item_file {
	...
	my ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids) = <FILE>;
	close FILE;
	chomp ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids);
	return ($title, $reserve, $inc, $desc, $image, $buynow, $buyout, @bids);
}
